...
public abstract class ReflectionUtils {
	...
	/**
	 * 通过bean的属性名称得到某个对象的属性的值(bean的类型很明确)
	 * 
	 * @param beanPropsName
	 *            ：为javabean属性的名称
	 * @return
	 */
	public static Object getBeanPropsValue(Object bean, String beanPropsName) {
		Object obj = null;
		try {
			Field[] fields = bean.getClass().getDeclaredFields();
			Field.setAccessible(fields, true);
			for (int i = 0; i < fields.length; i++) {
				if (beanPropsName.equals(fields[i].getName())) {
					try {
	...
	public static Field[] getAccessibleFields(final Object obj) {
		List<Field> r_fields = new ArrayList<Field>();
		for (Class<?> superClass = obj.getClass(); superClass != Object.class; superClass = superClass
				.getSuperclass()) {
			Field[] fields = superClass.getDeclaredFields();
			for (int i = 0; i < fields.length; i++) {
				fields[i].setAccessible(true);
				r_fields.add(fields[i]);
			}
		}
		...
		// return fields;
		// }
		return r_fields.toArray(new Field[0]);
	}
	...
	// KeyDisease key = new KeyDisease();
	// Field[] fields = ReflectionUtils.getAccessibleFields(key);
